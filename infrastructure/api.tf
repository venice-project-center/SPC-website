...
resource "aws_api_gateway_rest_api" "spc_file_api" {
  name = "spc_file_api"
}


#this adds the /files route to our api
resource "aws_api_gateway_resource" "spc_file_resource" {
  rest_api_id   = aws_api_gateway_rest_api.spc_file_api.id
  parent_id     = aws_api_gateway_rest_api.spc_file_api.root_resource_id
  path_part     = "add"
}
...
resource "aws_api_gateway_method" "files_get_method" {
  authorization = "NONE"
  http_method   = "POST"
  resource_id   = aws_api_gateway_resource.spc_file_resource.id
  rest_api_id   = aws_api_gateway_rest_api.spc_file_api.id
}
...
#resource "aws_api_gateway_integration_response" "spc_file_response" {
#  http_method = aws_api_gateway_method.files_get_method.http_method
#  resource_id = aws_api_gateway_resource.spc_file_resource.id
#  rest_api_id = aws_api_gateway_rest_api.spc_file_api.id
#  status_code = 200
